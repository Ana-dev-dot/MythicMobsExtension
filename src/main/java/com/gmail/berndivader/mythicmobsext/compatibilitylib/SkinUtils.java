		...
		private ProfileResponse(Player onlinePlayer) {
			...
			if (profileJson.isJsonObject()) {
				...
				try {
					...
					Multimap<String, Object> properties = (Multimap<String, Object>) class_GameProfile_properties
							.get(gameProfile);
					...
					plugin.getLogger().log(Level.WARNING, "Error serializing profile for " + onlinePlayer.getName(),
							ex);
				}
			}
						...
						for (int i = 0; i < propertiesJson.size(); i++) {
							...
							if (property != null && property.has("name") && property.has("value")) {
								...
								String signature = property.has("signature") ? property.get("signature").getAsString()
										: null;
								Object newProperty = class_GameProfileProperty_constructor.newInstance(name, value,
										signature);
					...
					try {
						if (playerCache.exists()) {
							...
							if (uuidJSON.isEmpty()) {
								engageHoldoff();
								if (DEBUG)
									plugin.getLogger().warning("Got empty UUID JSON for " + playerName);
							...
							if (uuidString == null) {
								engageHoldoff();
								if (DEBUG)
									plugin.getLogger().warning("Failed to parse UUID JSON for " + playerName);
								synchronizeCallback(callback, null);
								return;
							}
							if (DEBUG)
								plugin.getLogger().info("Got UUID: " + uuidString + " for " + playerName);
		...
		Bukkit.getScheduler().runTaskLaterAsynchronously(plugin, new Runnable() {
			...
			public void run() {
				...
				synchronized (lock) {
					...
					try {
						String profileJSON = fetchURL("https://sessionserver.mojang.com/session/minecraft/profile/"
								+ uuid.toString().replace("-", ""));
						if (profileJSON.isEmpty()) {
							synchronizeCallback(callback, null);
							engageHoldoff();
							if (DEBUG)
								plugin.getLogger().warning("Failed to fetch profile JSON for " + uuid);
							return;
						}
						if (DEBUG)
							plugin.getLogger().info("Got profile: " + profileJSON);
						JsonElement element = new JsonParser().parse(profileJSON);
						if (element == null || !element.isJsonObject()) {
							synchronizeCallback(callback, null);
							engageHoldoff();
							if (DEBUG)
								plugin.getLogger().warning("Failed to parse profile JSON for " + uuid);
							return;
						}
						...
						if (encodedTextures == null) {
							...
							engageHoldoff();
							if (DEBUG)
								plugin.getLogger().warning("Failed to find textures in profile JSON");
							return;
						}
						String decodedTextures = Base64Coder.decodeString(encodedTextures);
						if (DEBUG)
							plugin.getLogger().info("Decoded textures: " + decodedTextures);
						String skinURL = getTextureURL(decodedTextures);

						// A null skin URL here is normal if the player has no skin.
						if (DEBUG)
							plugin.getLogger()
									.info("Got skin URL: " + skinURL + " for " + profileJson.get("name").getAsString());
						ProfileResponse response = new ProfileResponse(uuid, profileJson.get("name").getAsString(),
								skinURL, profileJSON);
						synchronized (responseCache) {
							responseCache.put(uuid, response);
						}
