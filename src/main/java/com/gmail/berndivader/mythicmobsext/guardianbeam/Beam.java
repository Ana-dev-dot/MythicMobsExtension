...
public class Beam {
	 ...
	 * @param endingPosition   Position to stop the beam, or the position which the
	 *                         effect 'moves away from'.
	 * @param viewingRadius    Radius from either node of the beam from which it can
	 *                         be seen.
	 * @param updateDelay      Delay between checking if the beam should be hidden
	 *                         or shown to potentially applicable players.
	 */
	...
	/**
	 * Send the packets to create the beam to applicable players. This also starts
	 * the runnable which will make the effect visible if it becomes applicable to a
	 * player.
	 */
	public void start() {
	...
	/**
	 * Send the packets to remove the beam from the player, if applicable. This also
	 * stops the runnable.
	 */
	public void stop() {
		...
		for (UUID uuid : viewers) {
			...
			if (player != null && player.getWorld().getUID().equals(this.worldUID)
					&& isCloseEnough(player.getLocation())) {
	...
	/**
	 * Sets the starting position of the beam, or the position which the effect
	 * 'moves towards'.
	 * 
	 * @param location the starting position.
	 */
	public void setStartingPosition(Location location) {
		Preconditions.checkArgument(location.getWorld().getUID().equals(this.worldUID),
				"location must be in the same world as this beam");
		...
		while (iterator.hasNext()) {
			...
			if (player == null || !player.isOnline() || !player.getWorld().getUID().equals(this.worldUID)
					|| !isCloseEnough(player.getLocation())) {
				iterator.remove();
				continue;
			}
	...
	/**
	 * Sets the ending position of the beam, or the position which the effect 'moves
	 * away from'.
	 * 
	...
	public void setEndingPosition(Location location) {
		...
		while (iterator.hasNext()) {
			...
			if (!player.isOnline() || !player.getWorld().getUID().equals(this.worldUID)
					|| !isCloseEnough(player.getLocation())) {
				iterator.remove();
				continue;
			}
	...
	/**
	 * Checks if any packets need to be sent to show or hide the beam to any
	 * applicable player.
	 */
	public void update() {
		if (this.isActive) {
